let MarkUp = (function() {
	...
	function markUpLine(line, text) {
		let tokens = Lexer.scan(text ? text : line.innerText);
		line.innerHTML = '';
		...
		for (let t of tokens) {
			...
			switch (t.type) {
				case KEYWORD:
					item.classList.add('md-keyword');
					break;
				case NAME:
					if (startsWithUppercase(t.data)) item.classList.add('md-type');
					break;
				case NUMBER:
					item.classList.add('md-number');
					break;
				case STRING:
					item.classList.add('md-string');
					break;
				case SYMBOL:
					item.classList.add('md-symbol');
					break;
				case COMMENT:
					item.classList.add('md-comment');
					break;
				case META:
					item.classList.add('md-meta');
					break;
				case INVALID:
					item.classList.add('md-error');
					break;
			}
		}
